...
const SliderX = ({ industry, mTNum, yearsRange, toolsName }: any) => {
    return (
        <div className={`hidden ${toolsName === "Interactive Whiteboards" ? "w-[29%]" : "w-[20%]"} h-[20%] p-1 rounded-xl border-[1px] border-white/85 lg:flex justify-between items-center`} style={{ marginTop: mTNum }}>
...
const SliderXs = ({ toolsName, industry, mTNum, yearsRange }: any) => {
    return (
        <div className={`flex lg:hidden ${toolsName === "Interactive Whiteboards" ? "w-[90%] md:w-[50%]" : "w-[70%] md:w-[30%]"} h-[19%] p-1 rounded-xl border-[1px] border-white/85 justify-between items-center`} style={{ marginRight: mTNum }}>
            <div className="w-1/4 h-full flex justify-center items-center rounded-full">
                <Image className="w-[80%] h-[80%] object-cover rounded-full" src={KI} alt="" />
            </div>
...
const ToolsExp = () => {
    return (
        <div className="h-max py-9 lg:h-[70vh] w-full flex flex-col items-center">
